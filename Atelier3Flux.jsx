...
// ─────────────────────────────────────────────────────────────
function ModuleChefAtelier({ onSelectOf }) {
  const [ofs, setOfs] = useState([]);
  const [ofDetail, setOfDetail] = useState(null);
  ...
  const ouvrirOf = async (id) => {
    try {
      const { data } = await axios.get(`${API}/at3/of/${id}`);
      setOfDetail(data);
      setForm({
        at3_poids_cible_kg: data.at3_poids_cible_kg || '',
        at3_nb_bobines_cibles: data.at3_nb_bobines_cibles || '',
        at3_notes_regleur: data.at3_notes_regleur || '',
        at3_notes_chef: data.at3_notes_chef || '',
        at3_machine_assignee_id: data.at3_machine_assignee_id || '',
      });
    } catch { toast.error('Erreur chargement détail OF'); }
  };

  const sauvegarder = async (valider = false) => {
    setLoading(true);
    try {
      await axios.put(`${API}/at3/of/${ofDetail.id}/configurer`, { ...form, valider });
      toast.success(valider ? '✓ OF lancé en extrusion !' : '✓ Configuration sauvegardée');
      setOfDetail(null);
      charger();
    ...
    <div key={key}>
      ...
      {type === 'textarea' ? (
        <textarea value={form[key] || ''} onChange={e => setForm({ ...form, [key]: e.target.value })}
          ...
          style={{ width: '100%', border: '1px solid #d1d5db', borderRadius: 8, padding: 8, fontSize: 13, boxSizing: 'border-box', resize: 'vertical' }} />
      ) : (
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <button onClick={charger} style={{ background: '#f0fdf4', border: '1px solid #86efac', color: '#15803d', padding: '7px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 12 }}>
          🔄 Actualiser
        </button>
      </div>
      ...
      {ofDetail && (
        <div style={{ background: '#fff', borderRadius: 14, border: '2px solid #86efac', padding: 24, marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div>
              <div style={{ fontWeight: 800, fontSize: 17, color: '#14532d' }}>{ofDetail.numero_of}</div>
              <div style={{ fontSize: 13, color: '#6b7280' }}>{ofDetail.article_code} — {ofDetail.article_nom}</div>
              <div style={{ fontSize: 12, color: '#9ca3af' }}>Client : {ofDetail.client_nom} | Qté : {ofDetail.quantite}</div>
            </div>
            <Badge statut={ofDetail.at3_statut_zone} map={STATUT_OF} />
          </div>

          <BarreFlux statut_zone={ofDetail.at3_statut_zone} />

          {/* Fiche article */}
          <div style={{ background: '#f8fafc', borderRadius: 10, padding: 14, marginBottom: 14, fontSize: 12 }}>
            <div style={{ fontWeight: 700, color: '#1d4ed8', marginBottom: 8 }}>📄 Fiche Article</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 8 }}>
              {[
                ['Poids théorique', `${ofDetail.poids_theorique_kg || '—'} kg`],
                ['Cadence', `${ofDetail.cadence_theorique_kg_h || '—'} kg/h`],
                ['Couleur', ofDetail.couleur || '—'],
                ['Dimensions', `${ofDetail.longueur_mm || '—'} x ${ofDetail.largeur_mm || '—'} mm`],
              ].map(([l, v]) => (
                <div key={l} style={{ background: '#fff', borderRadius: 8, padding: '8px 10px', border: '1px solid #e5e7eb' }}>
                  <div style={{ fontSize: 10, color: '#6b7280' }}>{l}</div>
                  <div style={{ fontWeight: 700, color: '#374151' }}>{v}</div>
                </div>
              ))}
            </div>
            {ofDetail.composition?.length > 0 && (
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#374151', marginBottom: 6 }}>Composition :</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {(Array.isArray(ofDetail.composition) ? ofDetail.composition : []).map((c, i) => (
                    <span key={i} style={{ background: '#dbeafe', color: '#1d4ed8', padding: '2px 10px', borderRadius: 20, fontSize: 11 }}>
                      {c.code || c.designation} {c.pct ? `(${c.pct}%)` : ''} {c.quantite ? `${c.quantite}` : ''}
                    </span>
                  ))}
                </div>
              </div>
          ...
          {/* Formulaire config chef atelier */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12, marginBottom: 14 }}>
            {inp('Poids cible (kg)', 'at3_poids_cible_kg', 'number', 'ex: 500')}
            {inp('Nb bobines prévues', 'at3_nb_bobines_cibles', 'number', 'ex: 12')}
            {inp('Machine assignée (ID)', 'at3_machine_assignee_id', 'number')}
          </div>
          <div style={{ display: 'grid', gap: 10, marginBottom: 16 }}>
            {inp('Instructions régleur', 'at3_notes_regleur', 'textarea', 'Paramètres machine, températures, consignes...')}
            {inp('Notes chef atelier', 'at3_notes_chef', 'textarea', 'Notes internes...')}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => sauvegarder(true)} disabled={loading || ofDetail.at3_statut_zone === 'extrusion'}
              style={{ background: '#14532d', color: '#fff', border: 'none', padding: '9px 20px', borderRadius: 8, cursor: 'pointer', fontWeight: 700 }}>
              {loading ? '...' : '✅ Valider & Lancer Extrusion'}
            </button>
            <button onClick={() => setOfDetail(null)} style={{ background: '#f3f4f6', border: 'none', padding: '9px 14px', borderRadius: 8, cursor: 'pointer' }}>
              ✕ Fermer
            </button>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {ofs.map(o => {
          const sc = STATUT_OF[o.at3_statut_zone] || STATUT_OF['nouveau'];
          const pct = o.at3_poids_cible_kg > 0 ? Math.min(100, Math.round((o.poids_produit_kg / o.at3_poids_cible_kg) * 100)) : 0;
          return (
            <div key={o.id} style={{
              background: '#fff', borderRadius: 12, padding: '14px 18px',
              border: `2px solid ${sc.bg}`, display: 'flex', justifyContent: 'space-between',
              alignItems: 'center', flexWrap: 'wrap', gap: 10, cursor: 'pointer'
            }} onClick={() => ofDetail?.id === o.id ? setOfDetail(null) : ouvrirOf(o.id)}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4, flexWrap: 'wrap' }}>
                  <span style={{ fontWeight: 800, fontSize: 14, color: '#14532d' }}>{o.numero_of}</span>
                  <Badge statut={o.at3_statut_zone} map={STATUT_OF} />
                  {o.at3_composition_validee && (
                    <span style={{ background: '#dcfce7', color: '#15803d', padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700 }}>✓ Config validée</span>
                  )}
                </div>
                <div style={{ fontSize: 13, color: '#374151' }}>{o.article_code} — {o.article_nom}</div>
                <div style={{ fontSize: 11, color: '#9ca3af' }}>
                  Client : {o.client_nom} | Qté : {o.quantite} | Machine : {o.machine_code || '—'}
                </div>
                {o.at3_poids_cible_kg > 0 && (
                  <div style={{ marginTop: 6 }}>
                    <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 2 }}>
                      Progression : {parseFloat(o.poids_produit_kg || 0).toFixed(1)} / {o.at3_poids_cible_kg} kg — {pct}%
                    </div>
              ...
              <div style={{ textAlign: 'right', fontSize: 12, color: '#6b7280' }}>
                <div style={{ fontWeight: 700, color: '#14532d', fontSize: 14 }}>{o.nb_bobines || 0} bobines</div>
                <div>{o.nb_palettes || 0} palettes</div>
                <div style={{ marginTop: 6, color: '#1d4ed8', fontWeight: 600 }}>👁 Ouvrir ›</div>
              </div>
            </div>
          );
        })}
        {ofs.length === 0 && (
          <div style={{ background: '#fff', borderRadius: 14, padding: 48, textAlign: 'center', border: '1px solid #e5e7eb' }}>
            ...
            <p style={{ color: '#9ca3af' }}>Aucun OF en cours pour l'atelier 3</p>
          </div>
        )}
      </div>
    </div>
...
// ─────────────────────────────────────────────────────────────
// MODULE 2 : EXTRUSION — Saisie bobines opérateur
// ─────────────────────────────────────────────────────────────
