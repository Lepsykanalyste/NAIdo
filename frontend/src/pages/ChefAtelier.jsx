...
function GestionDevis() {
  ...
  const [form, setForm] = useState({
    ...
    notes:'', remise_pct:'0', remise_amt:'', taux_tva:'18'
  });
  ...
  const artPage = artPF.slice((pageProd-1)*PER_PAGE, pageProd*PER_PAGE);
  const totalPages = Math.ceil(artPF.length/PER_PAGE);
  ...
  const totalTVA = lignes.reduce((s,l)=>s+calcLigne(l).tva,0);
  const discAmt = parseFloat(form.remise_amt)||subtotalHT*parseFloat(form.remise_pct||0)/100;
  const totalTTC = subtotalHT+totalTVA-discAmt;
  ...
  const reset = () => {
    setLignes([]);
    setForm({client_id:'',date_validite:'',conditions_livraison:'',notes:'',remise_pct:'0',remise_amt:'',taux_tva:'18'});
    setClientInfo(null); setSearchArt('');
  };

  const sauver = async (statut) => {
    if(!form.client_id){toast.error('Client requis');return;}
    if(lignes.length===0){toast.error('Ajoutez au moins un article');return;}
    try {
      await axios.post(`${API}/devis`,{...form,lignes,statut_initial:statut});
      toast.success(statut==='brouillon'?'Brouillon sauvegardé !':'Devis créé !');
      setVue('liste'); reset(); charger();
  ...
  const transformerBC = async () => {
    try{
      ...
      toast.success('BC créé !');setShowTransfo(null);charger();
    }catch(e){toast.error(e.response?.data?.error||'Erreur');}
  };
  ...
  const inp = (extra={})=>({width:'100%',padding:'8px 10px',borderRadius:7,border:'1px solid #d1d5db',fontSize:13,...extra});
    ...
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <div style={{display:'flex',gap:10}}>
          {[
            {label:'Total',value:devis.length,color:'#0891b2',bg:'#e0f2fe',icon:'📋'},
            ...
            <div key={k.label} style={{background:k.bg,borderRadius:10,padding:'10px 16px'}}>
              <div style={{fontSize:10,color:'#6b7280',marginBottom:2}}>{k.icon} {k.label}</div>
              <div style={{fontSize:18,fontWeight:800,color:k.color}}>{k.value}</div>
            </div>
          ))}
        </div>
        <button onClick={()=>setVue('nouveau')} style={{background:'#0891b2',color:'#fff',border:'none',borderRadius:8,padding:'10px 20px',cursor:'pointer',fontWeight:700,fontSize:13}}>
          + Nouveau devis
        </button>
      </div>
      <div style={{background:'#fff',borderRadius:14,border:'1px solid #e5e7eb',overflow:'hidden'}}>
        {devis.length===0?(
          <div style={{padding:40,textAlign:'center',color:'#6b7280'}}>
            <div style={{fontSize:32,marginBottom:8}}>📋</div>Aucun devis
          </div>
        ):(
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
            <thead><tr style={{background:'#f0f9ff'}}>
              {['N° Devis','Client','Articles','Montant TTC','Validité','Statut','Actions'].map(h=>(
              ...
              {devis.map((d,i)=>(
                <tr key={d.id} style={{borderBottom:'1px solid #f3f4f6',background:i%2===0?'#fff':'#fafafa'}}>
                  ...
                  <td style={{padding:'9px 12px'}}>
                    <span style={{background:bgStatut(d.statut),color:couleurStatut(d.statut),padding:'2px 8px',borderRadius:20,fontSize:11,fontWeight:700}}>{labelStatut(d.statut)}</span>
                  </td>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:14,padding:24,width:440}}>
            ...
            {[['Réf. commande client','reference_client','text','ex: CMD-2026-123'],
              ...
              ['Adresse de livraison','adresse_livraison','text','Zone Industrielle, Abidjan'],
            ].map(([l,k,t,ph])=>(
      ...
      <div style={{display:'flex',gap:16,alignItems:'flex-start'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{background:'#fff',borderRadius:12,border:'1px solid #e5e7eb',padding:20}}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>Informations de la commande</div>
          ...
          <div style={{background:'#fff',borderRadius:12,border:'1px solid #e5e7eb',padding:20}}>
            ...
            <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))}
              rows={3} placeholder="Notes internes, instructions de livraison…"
              style={{width:'100%',padding:'9px 12px',border:'1px solid #d1d5db',borderRadius:8,fontSize:13,resize:'none'}}/>
          </div>
        </div>
            ...
            <div style={{padding:'8px 0',borderBottom:'1px solid #f3f4f6'}}>
              ...
              <div style={{display:'flex',gap:5}}>
                <input type="number" min="0" max="100" value={form.remise_pct}
                  onChange={e=>setForm(p=>({...p,remise_pct:e.target.value,remise_amt:''}))}
                  placeholder="%" style={{flex:1,padding:'5px 7px',border:'1px solid #e5e7eb',borderRadius:6,fontSize:12,textAlign:'right'}}/>
                <input type="number" min="0" value={form.remise_amt}
                  onChange={e=>setForm(p=>({...p,remise_amt:e.target.value,remise_pct:'0'}))}
...
function DemandesFabrication() {
  ...
  const [dfSelectionne, setDfSelectionne] = React.useState(null);
  const [dfMode, setDfMode] = React.useState('voir');
  const [dfQte, setDfQte] = React.useState('');
  const [dfDesc, setDfDesc] = React.useState('');
...
function DetailOF({ detail, machines, onClose, onRefresh, onStatut, setOngletActif }) {
  ...
  const chargerComposition = async () => {
    try {
      ...
      setConfigOf({at3_poids_cible_kg:detail.at3_poids_cible_kg?parseFloat(detail.at3_poids_cible_kg).toString():(detail.quantite_cible&&detail.poids_theorique_kg?Math.round(parseFloat(detail.quantite_cible)*parseFloat(detail.poids_theorique_kg)).toString():detail.quantite_cible?parseFloat(detail.quantite_cible).toString():''),at3_nb_bobines_cibles:detail.at3_nb_bobines_cibles||'',at3_notes_regleur:detail.at3_notes_regleur||'',at3_machine_assignee_id:detail.at3_machine_assignee_id||''});
  ...
  return (
    <div style={{background:'#fff',borderRadius:14,padding:20,marginBottom:16,border:'2px solid #0369a1'}}>
      ...
      {/* Actions statut */}
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
        {detail.statut==='planifie' && user?.role!=='chef_atelier' && <button onClick={()=>{onStatut(detail.id,'lance');onClose();}} style={{background:'#7c3aed',color:'#fff',border:'none',borderRadius:8,padding:'8px 16px',cursor:'pointer',fontSize:13,fontWeight:700}}>▶ Lancer</button>}
        {detail.statut==='lance' && detail.at3_statut_zone!=='extrusion' && user?.role!=='chef_atelier' && <button onClick={()=>{onStatut(detail.id,'en_cours');onClose();}} style={{background:'#d97706',color:'#fff',border:'none',borderRadius:8,padding:'8px 16px',cursor:'pointer',fontSize:13,fontWeight:700}}>⚙ Démarrer</button>}
        ...
        {detail.statut==='en_cours' && user?.role!=='chef_atelier' && <button onClick={()=>{onStatut(detail.id,'termine');onClose();}} style={{background:'#15803d',color:'#fff',border:'none',borderRadius:8,padding:'8px 16px',cursor:'pointer',fontSize:13,fontWeight:700}}>✓ Terminer</button>}
...
onClick={()=>window.open(`/api/of/${of.id}/pdf`,'_blank')}
                ...
                <td style={{padding:'9px 12px'}} onClick={e=>e.stopPropagation()}>
                  {false && <button onClick={()=>changerStatut(of.id,'lance')} style={{background:'#7c3aed',color:'#fff',border:'none',borderRadius:6,padding:'4px 10px',cursor:'pointer',fontSize:11,fontWeight:700}} title='Lancer cet OF en production'>▶ Lancer</button>}
                  {false && <button onClick={()=>changerStatut(of.id,'en_cours')} style={{background:'#d97706',color:'#fff',border:'none',borderRadius:6,padding:'4px 10px',cursor:'pointer',fontSize:11,fontWeight:700}} title='Démarrer la production'>⚙ Démarrer</button>}
                  {false && <button onClick={()=>changerStatut(of.id,'termine')} style={{background:'#15803d',color:'#fff',border:'none',borderRadius:6,padding:'4px 10px',cursor:'pointer',fontSize:11,fontWeight:700}} title='Marquer comme terminé'>✓ Terminer</button>}
...
function DashboardCommercial() {
  ...
  useEffect(() => {
    Promise.all([
      axios.get(`${API}/df`).catch(()=>({data:[]})),
      axios.get(`${API}/stock/resume`).catch(()=>({data:{}})),
    ]).then(([d,s])=>{
      setDfs(Array.isArray(d.data)?d.data:[]);
      setStock(s.data||{});
    }).finally(()=>setLoading(false));
  ...
  const dfEnAttente = dfs.filter(d=>d.statut==='en_attente').length;
  const dfValidees = dfs.filter(d=>d.statut==='validee').length;
  ...
  return (
    <div>
      ...
      <div style={{background:'#fff',borderRadius:14,border:'1px solid #e5e7eb',overflow:'hidden'}}>
        <div style={{padding:'14px 20px',borderBottom:'1px solid #e5e7eb',fontWeight:700,fontSize:14}}>📝 Mes demandes de fabrication</div>
        {loading ? <div style={{padding:20,textAlign:'center',color:'#6b7280'}}>Chargement...</div> :
        dfs.length===0 ? <div style={{padding:30,textAlign:'center',color:'#6b7280'}}>Aucune demande — créez la première via le menu</div> :
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
          <thead><tr style={{background:'#faf5ff'}}>
            {['N° DF','Article','Client','Quantité','Livraison','Statut','OF'].map(h=>(
              <th key={h} style={{padding:'8px 12px',textAlign:'left',fontWeight:600,color:'#7c3aed',borderBottom:'2px solid #e9d5ff'}}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {dfs.slice(0,10).map((df,i)=>(
              <tr key={df.id} style={{borderBottom:'1px solid #f3f4f6',background:i%2===0?'#fff':'#fafafa'}}>
                ...
                <td style={{padding:'8px 12px'}}>
                  <span style={{background:df.statut==='validee'?'#dcfce7':df.statut==='refusee'?'#fee2e2':'#fef3c7',color:df.statut==='validee'?'#15803d':df.statut==='refusee'?'#dc2626':'#d97706',padding:'2px 8px',borderRadius:20,fontSize:11,fontWeight:700}}>
                    {df.statut==='validee'?'✅ Validée':df.statut==='refusee'?'❌ Refusée':'⏳ En attente'}
                  </span>
                </td>
                <td style={{padding:'8px 12px',fontWeight:600,color:'#0369a1'}}>{df.numero_of||'—'}</td>
              </tr>
            ))}
          </tbody>
        </table>}
...
function OrdresLivraison() {
  ...
  const [bcChoisi, setBcChoisi] = useState(null);
  ...
  const charger = async () => {
    ...
    try {
      const [r1,r2,r3] = await Promise.all([
        ...
        axios.get(API+'/bc').catch(()=>({data:[]})),
        axios.get(API+'/stock/inventaire').catch(()=>({data:[]})),
      ]);
      setOls(r1.data||[]); setBcs(r2.data||[]); setStock(r3.data||[]);
    } finally { setLoading(false); }
  };
  ...
  const choisirBC = async (bc) => {
    ...
    try {
      ...
      setFormInfo(f=>({...f,adresse_livraison:bc.adresse_livraison||'',date_livraison_prevue:bc.date_livraison_souhaitee?bc.date_livraison_souhaitee.slice(0,10):''}));
      setStep(2);
  ...
  const clBC=s=>({recu:'#6b7280',confirme:'#0369a1',en_traitement:'#d97706',transforme_df:'#7c3aed',annule:'#dc2626'}[s]||'#6b7280');
  const bgBC=s=>({recu:'#f3f4f6',confirme:'#e0f2fe',en_traitement:'#fef3c7',transforme_df:'#f5f3ff',annule:'#fee2e2'}[s]||'#f3f4f6');
  const lbBC=s=>({recu:'Reçu',confirme:'Confirmé',en_traitement:'En traitement',transforme_df:'→ DF créée',annule:'Annulé'}[s]||s);
  const inp={width:'100%',padding:'8px 10px',borderRadius:7,border:'1px solid #d1d5db',fontSize:13,boxSizing:'border-box'};
  const couleurMode=mode==='stock'?'#15803d':'#0369a1';
  const bgMode=mode==='stock'?'#f0fdf4':'#eff6ff';
  ...
  return (
    <div style={{padding:'24px',maxWidth:1100,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        ...
        <div style={{display:'flex',gap:10}}>
          <button onClick={()=>ouvrirModal('stock')} style={{background:'#15803d',color:'#fff',border:'none',borderRadius:8,padding:'10px 16px',cursor:'pointer',fontSize:13,fontWeight:600}}>🏪 OL depuis stock</button>
          <button onClick={()=>ouvrirModal('bc')} style={{background:'#0369a1',color:'#fff',border:'none',borderRadius:8,padding:'10px 16px',cursor:'pointer',fontSize:13,fontWeight:600}}>📦 Transformer BC → OL</button>
        </div>
      </div>
        ...
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e5e7eb',overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style={{background:'#f9fafb'}}>
              {['N° OL','BC origine','Client','Articles','Livraison','Statut','Dérogatoire','Actions'].map(h=>(
            ...
            <tbody>
              {ols.length===0?(<tr><td colSpan={8} style={{padding:40,textAlign:'center',color:'#6b7280'}}>Aucun ordre de livraison</td></tr>):ols.map((ol,i)=>(
                <tr key={ol.id} style={{borderBottom:'1px solid #f3f4f6',background:i%2===0?'#fff':'#fafafa'}}>
                  ...
                  <td style={{padding:'9px 12px',fontSize:12,color:'#6b7280'}}>{ol.numero_bc||'—'}</td>
                  <td style={{padding:'9px 12px',fontSize:12}}>{ol.client_nom||'—'}</td>
                  <td style={{padding:'9px 12px',fontSize:12,textAlign:'center'}}><span style={{background:'#f3f4f6',borderRadius:12,padding:'2px 8px',fontWeight:600}}>{ol.nb_articles||0}</span></td>
      ...
      {showModal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'#fff',borderRadius:14,width:720,maxWidth:'98vw',maxHeight:'92vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            <div style={{padding:'20px 24px',overflowY:'auto',flex:1}}>
              {step===1&&(
                <div>
                  <div style={{fontWeight:700,fontSize:14,color:'#374151',marginBottom:12}}>Sélectionner un bon de commande</div>
                  <input value={searchBC} onChange={e=>setSearchBC(e.target.value)} placeholder="🔍 Rechercher par N° BC, client, référence..." style={{...inp,marginBottom:10}} autoFocus/>
                    ...
                    {bcsFiltres.length===0?(<div style={{padding:32,textAlign:'center',color:'#6b7280',fontSize:13}}>Aucun bon de commande trouvé</div>):bcsFiltres.map(bc=>(
                      <div key={bc.id} onClick={()=>!loadingBC&&choisirBC(bc)} style={{padding:'12px 16px',borderBottom:'1px solid #f3f4f6',cursor:loadingBC?'wait':'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',background:'#fff'}} onMouseEnter={e=>e.currentTarget.style.background=bgMode} onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
                        <div>
                          ...
                          {bc.reference_client&&<div style={{fontSize:11,color:'#6b7280'}}>Réf : {bc.reference_client}</div>}
                        </div>
                        <div style={{textAlign:'right',flexShrink:0,marginLeft:12}}>
                          <div style={{fontSize:11,color:'#6b7280',marginBottom:4}}>{bc.date_livraison_souhaitee?new Date(bc.date_livraison_souhaitee).toLocaleDateString('fr-FR'):'Pas de date'}</div>
                          <span style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:10,background:bgBC(bc.statut),color:clBC(bc.statut)}}>{lbBC(bc.statut)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {step===2&&bcChoisi&&(
                <div>
                  <div style={{background:bgMode,border:`1px solid ${brMode}`,borderRadius:8,padding:'10px 14px',marginBottom:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <div>
                      <span style={{fontWeight:800,color:couleurMode,fontSize:14}}>{bcChoisi.numero_bc}</span>
                      <span style={{fontSize:13,color:'#374151',marginLeft:10}}>{bcChoisi.client_nom}</span>
                      {bcChoisi.reference_client&&<span style={{fontSize:12,color:'#6b7280',marginLeft:8}}>· Réf: {bcChoisi.reference_client}</span>}
                    </div>
                    <button onClick={()=>{setStep(1);setBcChoisi(null);setLignesOL([]);}} style={{fontSize:12,color:'#6b7280',background:'#fff',border:'1px solid #e5e7eb',borderRadius:6,padding:'4px 10px',cursor:'pointer'}}>← Changer</button>
                  </div>
                  <div style={{fontWeight:700,fontSize:13,color:'#374151',marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <span>Articles à livrer <span style={{fontSize:11,color:'#6b7280',fontWeight:400,marginLeft:8}}>{mode==='stock'?'— quantités ajustées selon le stock':'— modifiez pour une livraison partielle'}</span></span>
                    <button onClick={()=>setLignesOL(p=>p.map(l=>({...l,inclure:true})))} style={{fontSize:11,color:couleurMode,background:'none',border:'none',cursor:'pointer',textDecoration:'underline'}}>Tout sélectionner</button>
                  </div>
                  <div style={{border:'1px solid #e5e7eb',borderRadius:8,overflow:'hidden',marginBottom:14}}>
                    <table style={{width:'100%',borderCollapse:'collapse'}}>
                      <thead><tr style={{background:'#f9fafb'}}>
                        <th style={{padding:'8px 10px',width:36,borderBottom:'1px solid #e5e7eb'}}></th>
                        <th style={{padding:'8px 10px',fontSize:11,fontWeight:600,color:'#6b7280',textAlign:'left',borderBottom:'1px solid #e5e7eb'}}>Article</th>
                        <th style={{padding:'8px 10px',fontSize:11,fontWeight:600,color:'#6b7280',textAlign:'right',borderBottom:'1px solid #e5e7eb'}}>Qté commandée</th>
                        {mode==='stock'&&<th style={{padding:'8px 10px',fontSize:11,fontWeight:600,color:'#6b7280',textAlign:'right',borderBottom:'1px solid #e5e7eb'}}>Stock dispo</th>}
                        <th style={{padding:'8px 10px',fontSize:11,fontWeight:700,color:couleurMode,textAlign:'right',borderBottom:'1px solid #e5e7eb'}}>Qté à livrer</th>
                      </tr></thead>
                      <tbody>
                        {lignesOL.length===0?(<tr><td colSpan={mode==='stock'?5:4} style={{padding:24,textAlign:'center',color:'#6b7280',fontSize:13}}>Aucune ligne dans ce BC</td></tr>):lignesOL.map((l,i)=>{
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setOlDetail(null)}>
          <div style={{background:'#fff',borderRadius:14,padding:28,width:500,maxWidth:'95vw'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              ...
              <div style={{display:'flex',gap:8}}>
                <button onClick={()=>window.open(`/api/ol/${olDetail.id}/pdf`,'_blank')} style={{background:'#e0f2fe',color:'#0891b2',border:'none',borderRadius:6,padding:'5px 12px',cursor:'pointer',fontSize:12,fontWeight:600}}>🖨 PDF</button>
                <button onClick={()=>setOlDetail(null)} style={{background:'none',border:'none',fontSize:20,cursor:'pointer',color:'#9ca3af'}}>✕</button>
              </div>
            </div>
    ...
    <div>
      ...
      <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <button onClick={()=>{ setShowForm(true); setLignes([]); }} style={{background:'#15803d',color:'#fff',border:'none',padding:'9px 20px',borderRadius:8,cursor:'pointer',fontWeight:700,whiteSpace:'nowrap'}}>
          {user?.role!=='commercial' ? '+ Nouvelle vente' : null}
        </button>
      </div>
