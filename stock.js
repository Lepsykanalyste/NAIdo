...
// ── INVENTAIRE — tous les articles (MP + PF + semi-finis...)
router.get('/inventaire', auth, async (req, res) => {
  try {
    ...
    res.json(rows);
  } catch(err) { console.error('stock/inventaire:',err.message); res.status(500).json({ error:err.message }); }
});
...
router.get('/lots', auth, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(q, params);
    res.json(rows);
  } catch(err) { console.error('stock/lots:',err.message); res.status(500).json({ error:err.message }); }
});
...
router.post('/lots', auth, async (req, res) => {
  ...
  try {
    ...
    const d = req.body;
    if (!d.article_id) return res.status(400).json({ error:'Article requis' });
    if (!d.numero_lot) return res.status(400).json({ error:'Numéro de lot requis' });
    if (!d.qte_initiale || parseFloat(d.qte_initiale)<=0) return res.status(400).json({ error:'Quantité requise' });
    ...
    `,[
      ...
      parseFloat(d.qte_initiale), parseFloat(d.prix_unitaire)||0,
      d.date_fabrication||null, d.date_dlc||null, d.date_dluo||null
    ]);

    // Mettre à jour stock_articles
    const emplId = d.emplacement_id || null;
    if (emplId) {
      await client.query(`
        ...
        DO UPDATE SET
          ...
          derniere_entree=NOW()
      `,[d.article_id, emplId, parseFloat(d.qte_initiale), parseFloat(d.prix_unitaire)||0]);
    }
    ...
    console.error('stock/lots POST:',err.message);
...
router.put('/lots/:id', auth, async (req, res) => {
  try {
    ...
    const valid = ['disponible','quarantaine','bloque','perime','epuise'];
...
router.post('/entree', auth, async (req, res) => {
  ...
  try {
    ...
    const qteParsed = parseFloat(qte);
    if (!qte || qteParsed <= 0) return res.status(400).json({ error:'Quantité invalide' });
    const prix = parseFloat(prix_unitaire)||0;

    // Créer ou mettre à jour le lot si numéro fourni
    if (numero_lot) {
      const { rows:existing } = await client.query(
        'SELECT id FROM lots_stock WHERE numero_lot=$1 AND article_id=$2',[numero_lot,article_id]
      );
      if (existing.length>0) {
        await client.query('UPDATE lots_stock SET qte_disponible=qte_disponible+$1 WHERE id=$2',[qteParsed,existing[0].id]);
      } else {
        await client.query(`
        ...
        `,[article_id,emplacement_id||null,numero_lot,qteParsed,prix,date_dlc||null]);
      }
    }
    ...
    if (emplacement_id) {
      await client.query(`
        ...
        DO UPDATE SET
          ...
          valeur_stock=stock_articles.valeur_stock+($3*$4),
          derniere_entree=NOW()
      `,[article_id,emplacement_id,qteParsed,prix]);
    } else {
      // Sans emplacement : utiliser le premier disponible
      const { rows:empls } = await client.query('SELECT id FROM emplacements_stock WHERE actif=true LIMIT 1');
      ...
      if (emplId) {
        ...
        `,[article_id,emplId,qteParsed]);
      }
    }

    await client.query('COMMIT');
    res.json({ success:true, message:`Entrée de ${qteParsed} enregistrée` });
  } catch(err) {
    await client.query('ROLLBACK');
    console.error('stock/entree:',err.message);
    res.status(500).json({ error:err.message });
  } finally { client.release(); }
});
...
router.post('/sortie', auth, async (req, res) => {
  ...
  try {
    ...
    // Vérifier stock
    const whereEmpl = emplacement_id ? 'AND emplacement_id=$2' : '';
    const paramsCheck = emplacement_id ? [article_id,emplacement_id] : [article_id];
    const { rows:stock } = await client.query(
      `SELECT COALESCE(SUM(qte_disponible),0) AS total FROM stock_articles WHERE article_id=$1 ${whereEmpl}`,
      paramsCheck
    );
    const disponible = parseFloat(stock[0]?.total||0);
    if (disponible < qteParsed) {
      await client.query('ROLLBACK');
      return res.status(400).json({ error:`Stock insuffisant : ${disponible.toFixed(3)} disponible, ${qteParsed} demandé` });
    }

    // Déduire stock_articles
    if (emplacement_id) {
      await client.query(
        ...
        [qteParsed,article_id,emplacement_id]
      );
    } else {
      await client.query(
        'UPDATE stock_articles SET qte_disponible=GREATEST(0,qte_disponible-$1),derniere_sortie=NOW() WHERE article_id=$2',
        [qteParsed,article_id]
      );
    }

    // FIFO sur les lots
    const { rows:lots } = await client.query(
      `SELECT id,qte_disponible FROM lots_stock WHERE article_id=$1 AND statut='disponible' AND qte_disponible>0 ORDER BY date_reception ASC,created_at ASC`,
      [article_id]
    );
    let reste = qteParsed;
    for (const lot of lots) {
      ...
      await client.query('UPDATE lots_stock SET qte_disponible=qte_disponible-$1 WHERE id=$2',[deduire,lot.id]);
      await client.query("UPDATE lots_stock SET statut='epuise' WHERE id=$1 AND qte_disponible<=0",[lot.id]);
      reste -= deduire;
    }

    await client.query('COMMIT');
    res.json({ success:true, message:`Sortie de ${qteParsed} enregistrée` });
  } catch(err) {
    await client.query('ROLLBACK');
    console.error('stock/sortie:',err.message);
    res.status(500).json({ error:err.message });
  } finally { client.release(); }
});

// ── MOUVEMENTS (journal des bons validés) ────────────────────
router.get('/mouvements', auth, async (req, res) => {
  try {
    ...
    // Si la table n'a pas encore de mouvements, retourner []
    console.error('stock/mouvements:',err.message);
    res.json([]);
  }
});
